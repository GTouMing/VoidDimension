package com.gtouming.void_dimension.component;

public class TagKeyName {
    public static String CURRENT_PAGE = "current_page";

    public static String SET_DAY_TIME = "set_day_time";
    public static String SET_WEATHER = "set_weather";

    public static String SET_RESPAWN_POINT = "set_respawn_point";
    public static String GET_RESPAWN_POINT = "get_respawn_point";
    public static String TELEPORT_TO_ANCHOR = "teleport_to_anchor";

    public static String SET_TELEPORT_TYPE = "set_teleport_type";
    public static String SET_GATHER_ITEMS = "set_gather_items";
    public static String OPEN_CONTAINER = "open_container";
    public static String OPEN_VOID_TERMINAL_FROM_CURIO = "open_void_terminal_from_curio";
}
